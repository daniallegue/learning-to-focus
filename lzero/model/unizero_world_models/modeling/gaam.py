"""
Gaussian Adaptive Attention Mechanism for Transformer World Model
"""
import math
...
class GAAM(Attention):
    ...
    Each head learns a mean offset (mu_p) and a variance (sigma_p) for a Gaussian
    ...
    def __init__(self, config: TransformerConfig) -> None:
        ...
        self.sigma_p = nn.Parameter(torch.full(
            (self.num_heads,), inv_softplus(init_sigma)
        ))
        # mean init (ensure non-negative via softplus)
        init_mu = getattr(config, 'init_adaptive_mu', 0.0)
        self.mu_p_raw = nn.Parameter(torch.full(
            (self.num_heads,), inv_softplus(init_mu)
        ))
    ...
    def forward(
        ...
        # apply Gaussian weights to scores
    ...
    def get_attention_map(
        ...
        """
        Compute the Gaussian-adaptive attention map for the input sequence,
        reusing kv_cache only for mask limits—without updating it.
        ...
        dist = dist.expand(self.num_heads, -1, -1)
