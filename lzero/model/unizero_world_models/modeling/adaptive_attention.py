...
class AdaptiveSpanAttention(Attention):
    ...
    def get_attention_map(
        ...
        """
        Compute the attention map for the input sequence, reusing an existing kv_cache
        purely to know how much past context to mask—without updating it again.
        """
        B, T, C = x.shape
        device = x.device

        # 1) project Q and (fresh) K exactly like forward
        q = self.query(x).view(B, T, self.num_heads, self.head_dim).transpose(1, 2)
        k_fresh = self.key(x).view(B, T, self.num_heads, self.head_dim).transpose(1, 2)

        # 2) apply rotary if used
        if getattr(self.config, 'rotary_emb', False) and freqs_cis is not None:
            q, k_fresh = apply_rotary_emb(q, k_fresh, freqs_cis=freqs_cis)

        # 3) read-only cache usage
        if kv_cache is not None:
            # do NOT call kv_cache.update()
            k, _ = kv_cache.get()  # (B, nh, L+T, head_dim)
            L = k.shape[2] - T
        else:
            k = k_fresh
            L = 0
        ...
        # 5) build causal mask (plus stale‐entry trimming)
        causal = self.causal_mask[L:L + T, :L + T]  # (T, total_len)
        if valid_ctx_len is not None:
            # trim off “stale” prefix per batch element
        ...
        spans = F.softplus(self.span_p).floor().clamp(max=self.max_len).long()  # (nh,)
        qpos = torch.arange(L, L + T, device=device).unsqueeze(1)  # (T,1)
        kpos = torch.arange(0, total_len, device=device).unsqueeze(0)  # (1, total_len)
        dist = (qpos - kpos).abs()  # (T, total_len)
        d_exp = dist.unsqueeze(0).expand(self.num_heads, -1, -1)  # (nh, T, total_len)
        s_exp = spans.unsqueeze(1).unsqueeze(2)  # (nh,1,1)
        adapt_mask = (d_exp <= s_exp).unsqueeze(0).expand(B, -1, -1, -1)  # (B,nh,T, total_len)

        # 7) combine masks & softmax
